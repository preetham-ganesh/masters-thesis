...
class Encoder(tf.keras.Model):
    def __init__(self, emb_size, rnn_size, vocab_size, rate):
        ...
        self.rnn = tf.keras.layers.LSTM(rnn_size, return_state=True, return_sequences=True)
...
class Decoder(tf.keras.Model):
    ...
    def call(self, inp, hidden, enc_out, training):
        ...
        inp = tf.concat([tf.expand_dims(context_vector, 1), inp], axis=-1)
        out, h, c = self.rnn(inp)
