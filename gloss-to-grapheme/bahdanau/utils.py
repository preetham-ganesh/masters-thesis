...
def text_retrieve(name):
    with open('/home/preetham/Documents/Preetham/masters-thesis/data/gloss-to-grapheme/'+name, 'r', encoding='utf-8') as f:
        text = f.read()
    f.close()
    return text.split('\n')

def open_file(name):
    loc_to = '/home/preetham/Documents/Preetham/masters-thesis/results/gloss-to-grapheme/bahdanau/'
...
def translate(gloss, model_name):
    ...
    sp.Load(loc_to + 'tokenizer/gloss-spt.model')
    gloss = sp.EncodeAsPieces(gloss)
    sequence = [[inp_word_index[i] for i in gloss]]
    ...
    english = []
    encoder, decoder = choose_encoder_decoder(parameters)
    checkpoint_dir = loc_to + 'bahdanau/model_' + str(parameters['model']) + '/training_checkpoints'
    checkpoint = tf.train.Checkpoint(encoder=encoder, decoder=decoder)
    checkpoint.restore(tf.train.latest_checkpoint(checkpoint_dir))
    ...
    sp.Load(loc_to + 'tokenizer/en-spt.model')
    english = sp.DecodePieces(english)
